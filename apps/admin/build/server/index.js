...
async function render_endpoint(event, mod, state) {
    ...
    if (prerender && (mod.POST || mod.PATCH || mod.PUT || mod.DELETE)) {
        throw new Error(
            'Cannot prerender endpoints that have mutative methods',
        );
    }
    if (
        state.prerendering &&
...
function has_data_suffix(pathname) {
    return (
        pathname.endsWith(DATA_SUFFIX) || pathname.endsWith(HTML_DATA_SUFFIX)
    );
}
    ...
    const universal_fetch = async (input, init2) => {
        ...
        if (same_origin) {
            ...
            if (mode === 'no-cors') {
                ...
                const acao = response.headers.get(
...
async function resolve_route(resolved_path, url, manifest) {
    ...
    return create_server_routing_response(route, params, url, manifest)
        .response;
}
...
async function render_response({
    ...
    let base_expression = s(base);
    {
        if (!state.prerendering?.fallback) {
            ...
            if (
                !assets ||
                (assets[0] === '/' && assets !== SVELTE_KIT_ASSETS)
            ) {
                assets$1 = base$1;
            }
    ...
    if (page_config.csr) {
        const route =
            manifest._.client.routes?.find((r) => r.id === event.route.id) ??
            null;
        ...
        if (client.uses_env_dynamic_public) {
            properties.push(
                `env: ${load_env_eagerly ? 'null' : s(public_env)}`,
            );
        }
        if (chunks) {
            ...
            thing
                ...
                .then(
                    ...
                    async ({ data, error }) => {
                        ...
                        const nonce = csp.script_needs_nonce
                            ? ` nonce="${csp.nonce}"`
                            : '';
...
class PageNodes {
    ...
    should_prerender_data() {
        return this.data.some(
            ...
            (node) =>
                node?.server?.load || node?.server?.trailingSlash !== void 0,
        );
    }
}
...
function get_cookies(request, url) {
    ...
    const cookies = {
        ...
        set(name, value, options2) {
            const illegal_characters = name.match(
...
function create_fetch({
    ...
    const server_fetch = async (info, init2) => {
        ...
        let mode =
            (info instanceof Request ? info.mode : init2?.mode) ?? 'cors';
        let credentials =
        ...
        return options2.hooks.handleFetch({
            ...
            fetch: async (info2, init3) => {
                ...
                if (info2 !== original_request) {
                    mode =
                        (info2 instanceof Request ? info2.mode : init3?.mode) ??
                        'cors';
                    credentials =
...
async function respond(request, options2, manifest, state) {
    ...
    const event = {
        ...
        getClientAddress:
            ...
            (() => {
                throw new Error(
                    `${'adapter-bun'} does not specify getClientAddress. Please raise an issue`,
                );
            }),
        locals: {},
        params: {},
                    ...
                    with_event(null, () =>
                        resolve2(event2, page_nodes, opts).then((response2) => {
                            ...
                            add_cookies_to_headers(
                                response2.headers,
                                Object.values(new_cookies),
                            );
                            if (
                                state.prerendering &&
                                event2.route.id !== null
                            ) {
                                response2.headers.set(
                                    'x-sveltekit-routeid',
    ...
    async function resolve2(event2, page_nodes, opts) {
        try {
            if (opts) {
                resolve_opts = {
                    transformPageChunk:
                        opts.transformPageChunk || default_transform,
            ...
            if (route) {
                ...
                if (is_data_request) {
                    ...
                    if (!page_nodes) {
                        throw new Error(
                            'page_nodes not found. This should never happen',
                        );
            ...
            try {
                ...
                set_app({
                    decoders: module.transport
                        ? Object.fromEntries(
                              Object.entries(module.transport).map(([k, v]) => [
                                  k,
                                  v.decode,
                              ]),
                          )
