            ...
            return {
                message:
                    (_a =
                        message !== null && message !== undefined
                            ? message
                            : required_error) !== null && _a !== undefined
                        ? _a
                        : ctx.defaultError,
            };
        }
        ...
        return {
            message:
                (_b =
                    message !== null && message !== undefined
                        ? message
                        : invalid_type_error) !== null && _b !== undefined
                    ? _b
                    : ctx.defaultError,
        };
    };
...
class ZodType {
                    ...
                    (_b =
                        (_a =
                            err === null || err === undefined
                                ? undefined
                                : err.message) === null || _a === undefined
                            ? undefined
                            : _a.toLowerCase()) === null || _b === undefined
                        ? undefined
                        : _b.includes('encountered')
                ) {
                    this['~standard'].async = true;
                }
                ctx.common = { issues: [], async: true };
            }
        }
    ...
    describe(description) {
        ...
        return new This({ ...this._def, description });
    }
    pipe(target) {
        ...
        return this._addCheck({
            ...
            local:
                (_b =
                    options === null || options === undefined
                        ? undefined
                        : options.local) !== null && _b !== undefined
                    ? _b
                    : false,
            ...errorUtil.errToObj(
                options === null || options === undefined
    ...
    return new ZodString({
        ...
        coerce:
            (_a =
                params === null || params === undefined
                    ? undefined
                    : params.coerce) !== null && _a !== undefined
                ? _a
                : false,
...
class ZodDate extends ZodType {
    _parse(input) {
        ...
        if (isNaN(input.data.getTime())) {
            const ctx2 = this._getOrReturnCtx(input);
            addIssueToContext(ctx2, { code: ZodIssueCode.invalid_date });
            return INVALID;
        }
        const status = new ParseStatus();
        let ctx = undefined;
    ...
    if (check)
        return ZodAny.create().superRefine((data, ctx) => {
            ...
            if (r instanceof Promise) {
                return r.then((r2) => {
                    ...
                    if (!r2) {
                        ...
                        const _fatal =
                            (_b2 =
                                (_a2 = params.fatal) !== null &&
                                _a2 !== undefined
                                    ? _a2
                                    : fatal) !== null && _b2 !== undefined
                                ? _b2
                                : true;
                        ctx.addIssue({
                            code: 'custom',
            ...
            if (!r) {
                ...
                const _fatal =
                    (_b =
                        (_a = params.fatal) !== null && _a !== undefined
                            ? _a
                            : fatal) !== null && _b !== undefined
                        ? _b
                        : true;
...
var env = createEnv({
    ...
    shared: { NODE_ENV: z.string().default('production') },
    runtimeEnv: Bun.env,
});
var env_default = env;
var SERVER_PATH = 'server';
async function buildKitServer(manifest, init_options) {
    const { Server } = __require(`./${SERVER_PATH}/index.js`);
    const server = new Server(manifest);
    await server.init({ env: Bun.env, ...init_options });
    return server;
}
...
async function getManifestFile() {
    try {
        const manifestPath = path.resolve(__dirname2(), 'manifest.js');
...
function get_origin(headers) {
    const protocol_header = env_default.PROTOCOL_HEADER.toLowerCase();
    const host_header = env_default.HOST_HEADER.toLowerCase();
    const port_header = env_default.PORT_HEADER.toLowerCase();
    const protocol =
...
function parseEnvBytes(input) {
    if (typeof input === 'number')
        return input === 0 ? Number.POSITIVE_INFINITY : input;
...
async function buildPrerenderRoutes(folderPath) {
    ...
    return Array.from(prerendered).map((v) => ({
        ...
        handler: async () => {
            try {
                const file = Bun.file(path2.join(folderPath, `${v}.html`));
                ...
                return Response.json(
                    {
                        code: 500,
                        message:
                            'Oops! Error occurred while serving static content.',
                        error: e.message,
                    },
                    { status: 500 },
                );
...
async function buildStaticRoute(folderPath, isClient = false) {
    ...
    const appHandler = {
        ...
        handler: async (req) => {
            try {
                ...
                const filepath = path3.join(folderPath, pathname);
    ...
    const assetsHandler = Array.from(assets).map((v) => ({
        route: `/${v}`,
        handler: async () => {
            try {
                const file = Bun.file(path3.join(folderPath, v));
                if (!(await file.exists())) {
                    return Response.json(
...
async function buildDisabledStaticRoute() {
    const { manifest } = await getManifestFile();
...
async function buildRoutes() {
    const routes = {};
    const staticFolderPath = path4.resolve(__dirname2(), 'client');
    const staticRoute = env_default.ASSETS
        ? await buildStaticRoute(staticFolderPath)
        : await buildDisabledStaticRoute();
    if (staticRoute) {
        staticRoute.map((v) => {
            routes[v.route] = v.handler;
        });
    }
    const prerenderFolderPath = path4.resolve(__dirname2(), 'prerendered');
...
function handleSSRRequest(originalRequest, bun_server, kit_server) {
    const baseOrigin =
        env_default.ORIGIN || get_origin(originalRequest.headers);
    const url = originalRequest.url.slice(
        originalRequest.url.split('/', 3).join('/').length,
    );
...
async function getBunServeConfig() {
    const { manifest } = await getManifestFile();
    const kitServer = await buildKitServer(manifest);
    return {
        port: env_default.PORT,
        hostname: env_default.HOST,
        maxRequestBodySize: parseEnvBytes(env_default.BODY_SIZE_LIMIT),
        development: env_default.DEV_MODE,
...
async function serve() {
    console.info(`Listening on ${env_default.HOST}:${env_default.PORT}`);
    Bun.serve(await getBunServeConfig());
}
var server_default = serve();
export { server_default as default };

//# debugId=7645821A4C32BDDE64756E2164756E21
